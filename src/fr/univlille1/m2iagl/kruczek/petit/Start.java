	...
	static class MyHandler implements HttpHandler {
		...
		public void handle(final HttpExchange httpExchange) throws IOException {
			...
			if (obj.has("pull_request")) {
				...
				final String diffUrlStr = obj.getJSONObject("pull_request").getString("diff_url");
				...
				String headContentUrl = obj.getJSONObject("pull_request").getJSONObject("head").getJSONObject("repo")
				...
				if (!pullState.equals("closed")) {
					...
					getCheckstyleFile(fileName, pullUrl.substring(0, pullUrl.indexOf("/pulls/")));
		...
		public static void getCheckstyleFile(final String fileName, final String repoUrl) {
			...
			try {
				...
				// xmlStr = xmlStr.replace("<module name=\"Checker\">","<module
				// name=\"Checker\">\n"+supComFilter);
				// xmlStr = xmlStr.replace("<module
				// name=\"TreeWalker\">","<module
				// name=\"TreeWalker\">\n"+fileConHolder);
		...
		public ArrayList<Fichier> getJavaDiffFile(String fileName, String diffUrlStr) throws IOException {
			final URL diffURL = new URL(diffUrlStr);
			final URLConnection diffUrlCon = diffURL.openConnection();
			final BufferedReader in = new BufferedReader(new InputStreamReader(diffUrlCon.getInputStream()));
			...
			while ((inputLine = in.readLine()) != null) {
				...
				if (inputLine.startsWith("diff --git") && inputLine.endsWith(".java")) {// commande
																						// diff
																						// sur
																						// les
																						// fichiers
				...
				} else if (inputLine.startsWith("diff --git") && !inputLine.endsWith(".java")) { // commande
																									// diff
																									// sur
																									// les
																									// autres
		...
		public static void commentPR(final String issueUrl, final String token, String outCheckFile) {
			...
			try {
				...
				// FileReader("tmp/out/"+outCheckFile+".check"));

				String filePath = new File("").getAbsolutePath();
				// System.out.println(filePath +
				// "/tmp/out/"+outCheckFile+".check");
				BufferedReader reader = new BufferedReader(
						new FileReader(filePath + "/tmp/out/" + outCheckFile + ".check"));
				String line = null;
				int nbl = 0;
